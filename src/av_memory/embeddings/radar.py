...
def radar_embed(signal: np.ndarray) -> list[float]:
    ...
    # Move to frequency domain; event signatures separate more clearly there.
    fft = np.fft.rfft(s)
    mag = np.abs(fft).astype(np.float32)

    # Downsample to a fixed number of bins to keep schema and runtime stable.
    bins = 128
    if mag.shape[0] < bins:
    ...
    # Clamp/pad to configured dimension so query and index vectors always match.
